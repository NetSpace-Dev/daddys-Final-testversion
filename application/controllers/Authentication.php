...
class Authentication extends Cl_Controller {
  ...
  public function setOutlet($outlet_id,$company_id){
            ...
            foreach ($printers as $ky=>$value){
                if(isset($value->id) && $value->id){
                    ...
                    if($sale_items){
                        ...
                        $previously_printed = $this->db->where('sales_id', $sale_details->id)->where('is_print', 2)->get('tbl_kitchen_sales_details')->num_rows() > 0;
                        $items = "\n";
                        $count = 1;
                        foreach ($sale_items as $item){
                            ...
                            if($qty):
                                ...
                                if($kot_print==2 && $previously_printed){
    ...
    public function printSaleBillByAjax(){
        ...
        if($sale_id){
            ...
            if(isset($company->receipt_printer_bill) && $company->receipt_printer_bill){
                ...
                $charge_to_print = 0;
                if (isset($sale->delivery_charge_actual_charge) && $sale->delivery_charge_actual_charge && $sale->delivery_charge_actual_charge!="0.00") {
                    $charge_to_print = $sale->delivery_charge_actual_charge;
                } else if (isset($sale->delivery_charge) && $sale->delivery_charge && $sale->delivery_charge!="0.00") {
                    $charge_to_print = $sale->delivery_charge;
                }
                $charge_label = "Charge";
                if (isset($sale->charge_type) && strtolower($sale->charge_type) == 'service') {
                    $charge_label = "Service Charge";
                } else if (isset($sale->charge_type) && strtolower($sale->charge_type) == 'delivery') {
                    $charge_label = "Delivery Charge";
                } else {
                    $charge_label = lang("Service_Delivery_Charge");
                }
                if($charge_to_print && $charge_to_print!="0.00"):
                    $totals.= printLine($charge_label. ": " .(($charge_to_print)), $printer_tmp->characters_per_line, ' ')."\n";
                endif;
        ...
        if ($new_ip) {
            ...
            if ($company_id) {
                ...
                if ($company) {
                    ...
                    if ($company->print_server_url_bill !== $new_ip) {
                        $update_data['print_server_url_bill'] = $new_ip;
                        $changed = true;
                    }
                    if ($company->print_server_url_kot !== $new_ip) {
                        $update_data['print_server_url_kot'] = $new_ip;
                        $changed = true;
                    }
                    if ($company->print_server_url_invoice !== $new_ip) {
    ...
    public function get_kot_print_data_by_sale_no_ajax(){
        $sale_no = htmlspecialcharscustom($this->input->post($this->security->xss_clean('sale_no')));
        
        $this->load->model('Sale_model');

        // Find all kitchens associated with this sale's items
        ...
        $this->db->select("tbl_kitchens.id");
        ...
        $kitchen_rows = $this->db->get()->result();
        ...
        if (empty($kitchen_ids)) {
            echo json_encode(['status' => 'error', 'message' => 'No kitchens/printers found for this sale']);
            return;
        }
        
        $kitchen_id_str = implode(",", $kitchen_ids);
        $printers = $this->Common_model->getSelectedPrinters($sale_no, $kitchen_id_str);
        
        $valid_printers = [];
        if($printers){
            foreach ($printers as $ky=>$value){
                $sale_items = $this->Common_model->getAllKitchenItems($sale_details->id,$value->id,2);
                foreach($sale_items as $single_item_by_sale_id){
                    $modifier_information = $this->Sale_model->getModifiersBySaleAndSaleDetailsIdKitchen($sale_details->id,$single_item_by_sale_id->sales_details_id);
                    $single_item_by_sale_id->modifiers = $modifier_information;
                }
        ...
        if (count($valid_printers) > 0) {
            ...
            $return_data['printer_server_url'] = "";
            $return_data['print_type'] = "KOT";
            $return_data['content_data'] = $valid_printers;
            $outlet_id = ($sale_details && isset($sale_details->outlet_id)) ? $sale_details->outlet_id : 1;
            $this->pushToPrintQueue("KOT", $valid_printers, $outlet_id);
            echo json_encode($return_data);
        } else {
            echo json_encode(['status' => 'no_change']);
        }
    ...
    public function mark_kot_as_printed_ajax() {
        $sale_no = htmlspecialcharscustom($this->input->post($this->security->xss_clean('sale_no')));
        $sale_details = getKitchenSaleDetailsBySaleNo($sale_no);
        if ($sale_details) {
            $update_kitchen['is_print'] = 2;
