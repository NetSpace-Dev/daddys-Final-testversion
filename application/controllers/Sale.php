...
class Sale extends Cl_Controller {
    ...
    public function POS($user_id='', $outlet_id=''){
        ...
        // Fetch all variations at once to prevent N+1 queries N*N memory leak
        $all_variations_raw = $this->db->query("SELECT * FROM tbl_food_menus WHERE parent_id != '0' AND del_status = 'Live'")->result();
        $variations_by_parent = array();
        if ($all_variations_raw) {
            foreach ($all_variations_raw as $v) {
                $variations_by_parent[$v->parent_id][] = $v;
            }
        }
        ...
        if(isset($data['food_menus']) && $data['food_menus']){
            foreach ($data['food_menus'] as $key=>$value){
                ...
                $data['food_menus'][$key]->is_variation = !empty($variations) ? 'Yes' : 'No';
                $data['food_menus'][$key]->variations = $variations;
                // kitchen_id and kitchen_name are already joined in the query!
            }
        }
    ...
    public function customer_display() {
        $this->load->view('sale/POS/customer_display');
    }
     /**
     * get Tables Details
     * @access public
    ...
    /**
     * Search customer by phone or name for quick delivery & item note auto-complete
     * @access public
     * @return json
     */
    public function search_customer_by_phone_ajax() {
        ...
        if (is_array($result)) {
            foreach ($result as $row) {
                $effective_address = ($row->latest_delivery_address && trim($row->latest_delivery_address) !== '') ? $row->latest_delivery_address : $row->address;
                $customers[] = array(
                    'id' => $row->id,
                    'name' => $row->name,
                    'phone' => $row->phone,
    ...
    /**
     * Quick add or update customer for delivery/takeaway/item note
     * @access public
     * @return json
     */
    public function quick_add_delivery_customer_ajax() {
        $customer_id = (int)$this->input->post('customer_id');
        $name = trim_checker(htmlspecialcharscustom($this->input->post('name')));
        $phone = trim_checker(htmlspecialcharscustom($this->input->post('phone')));
        $address = trim_checker(preg_replace('/\s+/', ' ', htmlspecialcharscustom($this->input->post('address'))));
        $company_id = $this->session->userdata('company_id');
        $user_id = $this->session->userdata('user_id');

        // If no details at all, return default Walk-in Customer (id: 1)
        if (!$phone && !$name && !$address && !$customer_id) {
            $walk_in = $this->db->get_where('tbl_customers', array('name' => 'Walk-in Customer', 'company_id' => $company_id))->row();
            $walk_id = $walk_in ? $walk_in->id : 1;
            echo json_encode(array(
                'status' => 'success',
                'customer' => array(
                    'id' => $walk_id,
        ...
        // Check if customer already exists with this phone if no ID provided
        if (!$customer_id && $phone) {
            $existing = $this->db->get_where('tbl_customers', array('phone' => $phone, 'del_status' => 'Live', 'company_id' => $company_id))->row();
            if ($existing) {
        ...
        // Manage active delivery address
        if ($address && $id_return > 0) {
            $this->db->where('customer_id', $id_return);
            $this->db->update('tbl_customer_address', array('is_active' => 0));
            ...
            $this->db->insert('tbl_customer_address', $addr_data);
        }
    ...
    public function add_kitchen_sale_by_ajax(){
        ...
        // Sync with tbl_sales if order already exists on server (e.g. placed by cashier)
        $existing_sale = $this->db->get_where('tbl_sales', array(
            'sale_no' => $sale_no,
        ...
        if ($existing_sale) {
            $sale_data = array(
                ...
                'total_item_discount_amount' => $data['total_item_discount_amount'],
                'sub_total_with_discount' => $data['sub_total_with_discount'],
                'sub_total_discount_amount' => $data['sub_total_discount_amount'],
                'total_discount_amount' => $data['total_discount_amount'],
                'delivery_charge' => $data['delivery_charge'],
                'sub_total_discount_value' => $data['sub_total_discount_value'],
                'sub_total_discount_type' => $data['sub_total_discount_type'],
                'orders_table_text' => $data['orders_table_text'],
                'waiter_id' => $data['waiter_id'],
                'order_status' => $data['order_status'],
                'modified' => 'Yes'
            );
            $this->db->where('id', $existing_sale->id);
            $this->db->update('tbl_sales', $sale_data);

            $this->db->delete('tbl_sales_details', array('sales_id' => $existing_sale->id));
            $this->db->delete('tbl_sales_details_modifiers', array('sales_id' => $existing_sale->id));
            $this->db->delete('tbl_sale_consumptions', array('sale_id' => $existing_sale->id));
            $this->db->delete('tbl_sale_consumptions_of_menus', array('sales_id' => $existing_sale->id));
            $this->db->delete('tbl_sale_consumptions_of_modifiers_of_menus', array('sales_id' => $existing_sale->id));
        }
        ...
        if($sale_id>0 && count($order_details->items)>0){
            ...
            foreach($order_details->items as $key_counter=>$item){
                $tmp_var_111 = isset($item->p_qty) && $item->p_qty !== 'undefined' && $item->p_qty !== '' ? (int)$item->p_qty : 0;
                $tmp = (int)$item->qty - $tmp_var_111;
                $tmp_var = 0;
                if($tmp > 0){
                    $tmp_var = $tmp;
                }
                ...
                if($sale_id){
                    ...
                    if(isset($check_exist_item) && $check_exist_item){
                        ...
                        if($item->qty!=$check_exist_item->qty){
                            ...
                            $diff_qty = (int)$item->qty - (int)$check_exist_item->qty;
                            $item_data['tmp_qty'] = $diff_qty > 0 ? $diff_qty : (int)$item->qty;
                ...
                $sale_sales_details_id = 0;
                if ($existing_sale) {
                    $food_details = $this->Common_model->getDataById($item->food_menu_id, "tbl_food_menus");
                    $p_name = '';
                    if(isset($food_details->parent_id) && $food_details->parent_id){
                        $p_name = getParentNameOnly($food_details->parent_id);
                    }
                    
                    $sale_item_data = array(
                        'food_menu_id' => $item->food_menu_id,
                        'menu_name' => $p_name.(isset($food_details->name) && $food_details->name?" ".$food_details->name:''),
                        'qty' => $item->qty,
                        'tmp_qty' => $item_data['tmp_qty'],
                        ...
                        'menu_taxes' => $item_data['menu_taxes'],
                        'menu_discount_value' => $item->menu_discount_value,
                        'discount_type' => $item->discount_type,
                        'menu_note' => $item_data['menu_note'],
                        'menu_combo_items' => $item_data['menu_combo_items'],
                        'discount_amount' => $item->item_discount_amount,
                        'item_type' => "Kitchen Item",
                        'cooking_status' => $item_data['cooking_status'],
                        'cooking_start_time' => $item_data['cooking_start_time'],
                        'cooking_done_time' => $item_data['cooking_done_time'],
                        'previous_id' => 0,
                    ...
                    if($order_details->customer_id!=1){
                        $sale_item_data['loyalty_point_earn'] = ($item->qty * getLoyaltyPointByFoodMenu($item->food_menu_id,''));
                    }
                    $this->db->insert('tbl_sales_details', $sale_item_data);
                    $sale_sales_details_id = $this->db->insert_id();
                    
                    $update_previous_id_sale = array('previous_id' => $sale_sales_details_id);
                    $this->db->where('id', $sale_sales_details_id);
                    $this->db->update('tbl_sales_details', $update_previous_id_sale);
                }
                ...
                if(!empty($modifier_id_array)>0){
                    ...
                    foreach($modifier_id_array as $key1=>$single_modifier_id){
                        ...
                        if ($existing_sale && $sale_sales_details_id > 0) {
                            $mod_data = array(
                                'modifier_id' => $single_modifier_id,
                                'modifier_price' => $modifier_price_array[$i],
                                'food_menu_id' => $item->food_menu_id,
                                'sales_id' => $existing_sale->id,
                                'sales_details_id' => $sale_sales_details_id,
                                'menu_taxes' => isset($modifier_vat_array[$key1]) && $modifier_vat_array[$key1]?$modifier_vat_array[$key1]:'',
                                'user_id' => $this->session->userdata('user_id'),
                                'outlet_id' => $this->session->userdata('outlet_id'),
                                'customer_id' => $order_details->customer_id
                            );
                            $this->db->insert('tbl_sales_details_modifiers', $mod_data);
                        }
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $is_takeaway = (isset($order_details->order_type) && $order_details->order_type == 2);
            $is_printing_return = 1;
            $valid_printers = array();
            if($printers){
                foreach ($printers as $ky=>$value){
                    if(isset($value->id) && $value->id){
                        ...
                        if($sale_items){
                            ...
                            $cust_id = isset($order_details->customer_id) ? $order_details->customer_id : (isset($data['customer_id']) ? $data['customer_id'] : '');
                            $printers[$ky]->customer_name = (isset($order_details->customer_name) && !empty($order_details->customer_name)) ? $order_details->customer_name : getCustomerName($cust_id);
                            $printers[$ky]->customer_address = (isset($order_details->customer_address) && !empty($order_details->customer_address)) ? $order_details->customer_address : getCustomerAddress($cust_id);
                            ...
                            foreach ($sale_items as $item){
                                if($item->tmp_qty):
                                    ...
                                    if($is_modified){
                                        $items.= "   NEW ITEMS - EXISTING ORDER\n";
                                    }
                                    $count++;
                            ...
                            if ($count > 1) {
                                ...
                                $valid_printers[] = $printers[$ky];
                            }
                        }
                    }
                }
                
                $printers = array_values($valid_printers);
                if (count($printers) > 0) {
                    $is_printing_return++;
                }
                ...
                $web_type = $company ? $company->printing_kot : '';
                ...
                if($web_type=="web_browser"){
                    ...
                    if($is_printing_return!=1 && !empty($printers)){
                    ...
                    if($is_printing_return!="live_server_print"){
                        ...
                        $return_data['content_data'] = $printers ? $printers : array();
                        $return_data['print_type'] = "KOT";
                        if(!empty($printers)){
                            $this->pushToPrintQueue("KOT", $printers);
                        }
                        echo json_encode($return_data);
                    }
                }
            } else {
                // If no printers are configured, still return success so the order is saved normally
    ...
    public function push_online(){
        ...
        if (trim_checker($order_details->order_status) == 3) {
            // Closed / Invoiced order - clean up active table bookings and kitchen sales
            $this->db->delete('tbl_orders_table', array('sale_id' => $sales_id));
            $this->db->delete('tbl_orders_table', array('sale_no' => $sale_no));
            $this->db->delete('tbl_running_order_tables', array('sale_no' => $sale_no));
            
            $kitchen_row = $this->db->get_where('tbl_kitchen_sales', array('sale_no' => $sale_no))->row();
            if ($kitchen_row) {
                $this->db->delete('tbl_kitchen_sales_details', array('sales_id' => $kitchen_row->id));
                $this->db->delete('tbl_kitchen_sales_details_modifiers', array('sales_id' => $kitchen_row->id));
                $this->db->delete('tbl_kitchen_sales', array('id' => $kitchen_row->id));
            }
        } else {
            if (isset($order_details->orders_table) && is_array($order_details->orders_table)) {
                foreach($order_details->orders_table as $single_order_table){
                    $order_table_info = array();
                    $order_table_info['persons'] = $single_order_table->persons;
                    $order_table_info['booking_time'] = date('Y-m-d H:i:s');
        ...
        $is_closed_order = (trim_checker($order_details->order_status) == 3);
        // $already_synced_to_db: TRUE means the sale already existed in MySQL before this push_online call.
        // If $sale_id > 0, consumption data was written during a previous sync (when it was a running order),
        // so it's safe to skip re-processing for closed orders (avoids 30-50 redundant DB queries).
        // If $sale_id == 0, this is the FIRST sync (e.g. invoiced while offline) — full processing is required
        // to prevent data loss of stock consumptions and modifier ingredients.
        $already_synced_to_db = ($sale_id > 0);
        $skip_consumptions = ($is_closed_order && $already_synced_to_db);

        // Only insert consumption records when NOT skipping (active orders or first-time closed order sync).
        if (!$skip_consumptions) {
            $data_sale_consumptions = array();
            $data_sale_consumptions['sale_id'] = $sales_id;
            $data_sale_consumptions['user_id'] = $this->session->userdata('user_id');
            $data_sale_consumptions['outlet_id'] = $this->session->userdata('outlet_id');
        ...
        if($sales_id>0 && count($order_details->items)>0){
            foreach($order_details->items as $item){
                ...
                $item_data['menu_name'] = $p_name.(isset($food_details->name) && $food_details->name?" ".$food_details->name:'');
                $item_data['qty'] = $item->qty;
                ...
                // Skip expensive ingredient consumption sub-queries when safe to do so.
                // Safe to skip only if: order is closed (status=3) AND was already in MySQL before this push
                // (meaning consumption data was written during the running-order sync).
                // For offline-invoiced orders syncing for the first time ($sale_id was 0), full processing is required.
                if (!$skip_consumptions) {
                ...
                } // end if (!$skip_consumptions) — ingredient consumption block

                // tbl_sales_details_modifiers MUST always be written (not guarded by $skip_consumptions)
                // because push_online deletes old modifier rows before re-inserting tbl_sales_details,
                // so skipping this would permanently wipe modifier data from the DB for already-synced closed orders.
                // Only the modifier_ingredient sub-consumption query inside is skipped for already-synced closed orders.
                ...
                if(!empty($modifier_id_array)>0){
                    ...
                    foreach($modifier_id_array as $key1=>$single_modifier_id){
                        ...
                        // Skip modifier ingredient consumption sub-queries for already-synced closed orders only.
                        if (!$skip_consumptions) {
                        ...
                        } // end if (!$skip_consumptions) — modifier ingredients consumption
                        $i++;
                    }
                }
            }
...
We hope to see you again!";
            ...
            if($select_kitchen_row){
                ...
                if($pre_or_post_payment==1){
                    ...
                    if ($order_details->order_status == 3) {
                        ...
                        $designation = $this->session->userdata('designation');
                        if($role=="Admin"){
                            $kitchen_data['pull_update_admin'] = 3;
                            $kitchen_data['pull_update'] = 2; // For Waiters
                            $kitchen_data['pull_update_cashier'] = 2; // For other Cashiers
                            $kitchen_data['is_delete_receiver'] = 1;
                        }else{
                            if($designation=="Cashier"){
                                $kitchen_data['pull_update_cashier'] = 3;
                                $kitchen_data['pull_update'] = 2; // For Waiters
                                $kitchen_data['pull_update_admin'] = 2; // For Admins
                            }else{
                                $kitchen_data['pull_update'] = 3; // For Waiters
                                $kitchen_data['pull_update_cashier'] = 2;
                                $kitchen_data['pull_update_admin'] = 2;
                            }
                        }
                        $this->db->where('id', $select_kitchen_row->id);
                        $this->db->update('tbl_kitchen_sales', $kitchen_data);
                    }
                }
            }
    ...
    public function get_active_dine_in_orders_with_details_ajax(){
        $outlet_id = $this->session->userdata('outlet_id');
        ...
        $running_sales = $this->db->get()->result();
        
        $detailed_sales = array();
        if ($running_sales) {
            foreach ($running_sales as $sale) {
                // Check if this order is already closed/paid in tbl_sales
                $closed_sale = $this->db->query("SELECT id FROM tbl_sales 
                    WHERE (sale_no = " . $this->db->escape($sale->sale_no) . " OR id = " . $this->db->escape($sale->id) . ") 
                    AND outlet_id = " . $this->db->escape($outlet_id) . " 
                    AND order_status = 3 
                    AND del_status = 'Live'")->row();
                
                if ($closed_sale) {
                    $this->db->delete('tbl_kitchen_sales_details', array('sales_id' => $sale->id));
                    $this->db->delete('tbl_kitchen_sales', array('id' => $sale->id));
                    continue;
                }

                $sale_info = $this->get_all_information_of_a_sale($sale->sale_id);
                if ($sale_info) {
                    $sale_info->sale_id = $sale->sale_id;
                }
                $detailed_sales[] = $sale_info;
            }
        }
        
        $response = array(
            'status' => 'success',
            'orders' => $detailed_sales
        );
    ...
    public function get_all_information_of_a_sale($sales_id){
        if (is_string($sales_id) && strpos($sales_id, 'srv_') === 0) {
            $sales_id = substr($sales_id, 4);
        }
        $sales_information = $this->get_all_information_of_a_sale_kitchen($sales_id);
        if (!$sales_information) {
            $sales_inf_arr = $this->Sale_model->getSaleBySaleId($sales_id);
            if (!empty($sales_inf_arr)) {
                $sales_information = $sales_inf_arr[0];
            } else {
                $sales_information = $this->db->get_where('tbl_sales', array('sale_no' => $sales_id))->row();
            }
        }
        ...
        if ($sales_information) {
            $sales_information->user_name = userName($sales_information->user_id);
            $sales_information->user_designation = userDesignation($sales_information->user_id);
            $self_order_content = isset($sales_information->self_order_content) && $sales_information->self_order_content ? json_decode($sales_information->self_order_content) : null;
            if ($self_order_content) {
                foreach ($self_order_content as $key => $value) {
                    if ($key !== 'items' && !isset($sales_information->$key)) {
                        $sales_information->$key = $value;
                    }
                }
            }
        }
        ...
        $this_value = isset($sales_information->sub_total_discount_value) ? $sales_information->sub_total_discount_value : '';
        ...
        if (is_array($items_by_sales_id)) {
            foreach($items_by_sales_id as $single_item_by_sale_id){
                ...
                $mod_names = array();
                $mod_prices = array();
                if ($modifier_information) {
                    foreach ($modifier_information as $mod) {
                        $mod_ids[] = $mod->modifier_id;
                        $mod_names[] = $mod->name;
                        $mod_prices[] = $mod->price;
                    }
                }
                $single_item_by_sale_id->modifiers_id = implode(',', $mod_ids);
                $single_item_by_sale_id->modifiers_name = implode(',', $mod_names);
                $single_item_by_sale_id->modifiers_price = implode(',', $mod_prices);
            }
        }
        $sales_details_objects = $items_by_sales_id;
        if (!empty($sales_details_objects) && isset($sales_details_objects[0])) {
            ...
            $this_value = isset($sales_details_objects[0]->menu_discount_value) ? $sales_details_objects[0]->menu_discount_value : '';
            $disc_fields = explode('%',$this_value);
            $discP = isset($disc_fields[1]) && $disc_fields[1]?$disc_fields[1]:'';
            if ($discP == "") {
            } else {
                $sales_details_objects[0]->menu_discount_value = getAmtP(isset($sales_details_objects[0]->menu_discount_value) && isset($sales_information->menu_discount_value) && $sales_information->menu_discount_value?$sales_details_objects[0]->menu_discount_value:0);
            }
        }
        ...
        $sale_no_chk = isset($sales_information->sale_no) ? $sales_information->sale_no : '';
        $unprinted_cnt = $this->db->query("
            SELECT COUNT(ksd.id) as total 
            FROM tbl_kitchen_sales_details ksd
            LEFT JOIN tbl_kitchen_sales ks ON ks.id = ksd.sales_id
            WHERE (ks.sale_no = " . $this->db->escape($sale_no_chk) . " OR ksd.sales_id = " . $this->db->escape($sales_id) . ") 
              AND ksd.is_print = 1
        ")->row();
        $sale_object->has_unprinted_kot = ($unprinted_cnt && $unprinted_cnt->total > 0) ? 1 : 0;
        return $sale_object;
    }
    public function get_all_information_of_a_sale_kitchen($sales_id){
        ...
        if (!$sales_information) {
            $sales_information = $this->db->get_where('tbl_kitchen_sales', array('sale_no' => $sales_id))->row();
        }
    ...
    public function get_all_information_of_a_sale_modify($sales_id){
        ...
        if (empty($sales_information)) {
            $sales_information = $this->db->get_where('tbl_sales', array('sale_no' => $sales_id))->result();
        }
        if (empty($sales_information)) {
            $ks_info = $this->Common_model->getDataById($sales_id, "tbl_kitchen_sales");
            if (!$ks_info) {
                $ks_info = $this->db->get_where('tbl_kitchen_sales', array('sale_no' => $sales_id))->row();
            }
            if ($ks_info) {
                $sales_information = array($ks_info);
            }
        }

        if (empty($sales_information) || !isset($sales_information[0])) {
            return null;
        }
        ...
        $this_value = isset($sales_information[0]->sub_total_discount_value) ? $sales_information[0]->sub_total_discount_value : '';
        $disc_fields = explode('%',$this_value);
        $discP = isset($disc_fields[1]) && $disc_fields[1]?$disc_fields[1]:'';
          if ($discP == "") {
          } else {
        ...
        if (is_array($items_by_sales_id)) {
            foreach($items_by_sales_id as $single_item_by_sale_id){
                ...
                $free_items = $this->Sale_model->getAllItemsFromSalesDetailBySalesIdModifyChild(isset($single_item_by_sale_id->id) ? $single_item_by_sale_id->id : 0,$sales_id);
        ...
        if (!empty($sales_details_objects) && isset($sales_details_objects[0])) {
            ...
            if ($discP == "") {
                ...
                $sales_details_objects[0]->menu_discount_value = getAmtP(isset($sales_details_objects[0]->menu_discount_value) && isset($sales_information[0]->menu_discount_value) && $sales_information[0]->menu_discount_value?$sales_details_objects[0]->menu_discount_value:0);
            }
        }
    ...
    /**
     * print KOT by sale_no (for waiter app orders pulled into cashier screen)
     * @access public
     * @return void
     * @param string $sale_no
     */
    public function print_kot_by_sale_no($sale_no){
        $sale_no = urldecode($sale_no);
        $sale_row = $this->db->get_where('tbl_kitchen_sales', array('sale_no' => $sale_no, 'del_status' => 'Live'))->row();
        if (!$sale_row) {
            $sale_row = $this->db->get_where('tbl_sales', array('sale_no' => $sale_no, 'del_status' => 'Live'))->row();
        }
        if (!$sale_row) {
            show_error('Sale not found for sale_no: ' . htmlspecialchars($sale_no));
            return;
        }
        $sale_id = $sale_row->id;
        // Mark items as printed when KOT print window opens
        $update_kitchen = array('is_print' => 2);
        $this->db->where('sales_id', $sale_id);
        $this->db->update('tbl_kitchen_sales_details', $update_kitchen);
        $this->db->update('tbl_kitchen_sales_details_modifiers', $update_kitchen);
    ...
    public function update_order_status_ajax()
    {
        ...
        $sale_id_param = $this->input->post('sale_id');
        $sale_no_param = $this->input->post('sale_no');
        ...
        if (is_string($sale_id_param) && strpos($sale_id_param, 'srv_') === 0) {
            $sale_id_param = substr($sale_id_param, 4);
        }

        // Resolve sale by sale_id or sale_no
        $sale_details = null;
        if (is_numeric($sale_id_param) && $sale_id_param > 0) {
            $sale_details = $this->Common_model->getDataById($sale_id_param, "tbl_sales");
        }
        if (!$sale_details && $sale_no_param) {
            $sale_details = $this->db->get_where('tbl_sales', array('sale_no' => $sale_no_param, 'del_status' => 'Live'))->row();
        }
        if (!$sale_details && $sale_id_param) {
            $sale_details = $this->db->get_where('tbl_sales', array('sale_no' => $sale_id_param, 'del_status' => 'Live'))->row();
        }

        $sale_id = $sale_details ? $sale_details->id : $sale_id_param;
        $sale = $sale_details;
        ...
        if($currency_type==1){
            ...
            $data['multi_currency_rate'] = $multi_currency_rate;
            $data['currency_type'] = $currency_type;
            $data['date_time'] = isset($sale->date_time) ? $sale->date_time : date('Y-m-d H:i:s');
            ...
            if (is_array($payment_details) && is_numeric($sale_id) && $sale_id > 0) {
                foreach ($payment_details as $value){
        ...
        if((int)$sub_total_discount_finalize && $sale_details){
        ...
        $is_close_order = ($close_order === 'true' || $close_order === true || $close_order == 1 || $close_order === '1');
        if($is_close_order){
            if (is_numeric($sale_id) && $sale_id > 0) {
                $this->Sale_model->delete_status_orders_table($sale_id);
            }
            $target_sale_no = ($sale_details && isset($sale_details->sale_no)) ? $sale_details->sale_no : $sale_no_param;
            if ($target_sale_no) {
                ...
                $select_kitchen_row = getKitchenSaleDetailsBySaleNo($target_sale_no);
                if ($select_kitchen_row) {
                    $this->db->delete("tbl_kitchen_sales_details", array("sales_id" => $select_kitchen_row->id));
                    $this->db->delete("tbl_kitchen_sales_details_modifiers", array("sales_id" => $select_kitchen_row->id));
                    $this->db->delete("tbl_kitchen_sales", array("id" => $select_kitchen_row->id));
            ...
            if (is_numeric($sale_id_param) && $sale_id_param > 0) {
                $this->db->delete("tbl_kitchen_sales_details", array("sales_id" => $sale_id_param));
                $this->db->delete("tbl_kitchen_sales_details_modifiers", array("sales_id" => $sale_id_param));
                $this->db->delete("tbl_kitchen_sales", array("id" => $sale_id_param));
            }
            if($is_just_cloase){
        ...
        } else if ($target_sale_no) {
            $this->db->where('sale_no', $target_sale_no);
            $this->db->update('tbl_sales', $order_status);
        }
    ...
    public function getWaiterOrders(){
        ...
        $unprinted_waiter_orders = $this->Common_model->getUnprintedWaiterOrders();
        ...
        $return_data['unprinted_waiter_orders'] = $unprinted_waiter_orders;
    ...
    public function get_sale_id_by_sale_no_ajax() {
        $sale_no = $this->input->post('sale_no');
        
        // 1. Search in tbl_kitchen_sales (running orders)
        $this->db->select('id');
        $this->db->from('tbl_kitchen_sales');
        ...
        if ($query) {
            echo json_encode(array('status' => 'success', 'sale_id' => $query->id));
        } else {
            // 2. Search in tbl_sales (finalized orders)
            $this->db->select('id');
    ...
    public function removePulledData(){
        ...
        $running_order = $this->db->get_where('tbl_running_orders', array('id' => $id))->row();
        if ($running_order) {
            $this->db->delete('tbl_running_order_tables', array('sale_no' => $running_order->sale_no));
        }
    ...
    public function getAjaxData() {
        try {
            ...
            foreach ($sales as $value){
                if($value->del_status=="Live"):
                   ...
                   if($order_type=="Delivery"):
                    $html .= '<li data-access="change_delivery_address-123" class="menu_assign_class"><a class="ir_mouse_pointer change_delivery_details" data-status="'.escape_output($value->status).'" data-id="'.escape_output($value->id).'"><i
            ...
            if (!empty($db_error['message'])) {
                file_put_contents('ajax_error.log', "DB Error: " . $db_error['message'] . "\n", FILE_APPEND);
                throw new Exception("DB Error: " . $db_error['message']);
            }
            ...
            file_put_contents('ajax_error.log', $t->getMessage() . "\n" . $t->getTraceAsString() . "\n", FILE_APPEND);
            echo json_encode(["error" => $t->getMessage(), "trace" => $t->getTraceAsString()]);
        }
    }

    public function debug_kot_status($sale_no = null) {
        if (!$sale_no) {
        ...
        $report = array();
        $report['sale_no_queried'] = $sale_no;
        $report['session_outlet_id'] = $outlet_id;
        $report['session_company_id'] = $company_id;

        // 1. Check tbl_sales
        $sale_tbl = $this->db->get_where('tbl_sales', array('sale_no' => $sale_no))->result();
        $report['tbl_sales_records'] = $sale_tbl;

        // 2. Check tbl_kitchen_sales
        $ks_tbl = $this->db->get_where('tbl_kitchen_sales', array('sale_no' => $sale_no))->result();
        $report['tbl_kitchen_sales_records'] = $ks_tbl;

        // 3. Check helper getKitchenSaleDetailsBySaleNo
        $helper_res = getKitchenSaleDetailsBySaleNo($sale_no);
        $report['getKitchenSaleDetailsBySaleNo_result'] = $helper_res ? $helper_res : 'FAILED_OR_FALSE';

        // 4. Check tbl_kitchen_sales_details items
        if ($ks_tbl && count($ks_tbl) > 0) {
            $ks_id = $ks_tbl[0]->id;
            $items = $this->db->get_where('tbl_kitchen_sales_details', array('sales_id' => $ks_id))->result();
            $report['tbl_kitchen_sales_details_items'] = $items;

            $unprinted_items = $this->db->get_where('tbl_kitchen_sales_details', array('sales_id' => $ks_id, 'is_print' => 1))->result();
            $report['unprinted_is_print_1_count'] = count($unprinted_items);
        } else {
            $report['tbl_kitchen_sales_details_items'] = 'No tbl_kitchen_sales record found';
        }

        // 5. Check printer config
        $company = $this->Common_model->getDataById($company_id, "tbl_companies");
        $report['company_printing_kot_setting'] = isset($company->printing_kot) ? $company->printing_kot : 'not set';

        header('Content-Type: application/json');
        echo json_encode($report, JSON_PRETTY_PRINT);
    }

    private function pushToPrintQueue($print_type, $content_data, $outlet_id = null) {
        if (!$outlet_id) {
