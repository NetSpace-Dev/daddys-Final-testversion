...
class Waiter_app extends Cl_Controller {
    ...
    public function add_sale_by_ajax(){
        ...
        if (!$sale_id && isset($order_details->sale_id)) {
            $sale_id = $order_details->sale_id;
        }
        $user_id = $this->input->post('user_id');
        $outlet_id = $this->input->post('outlet_id');
        if (!$sale_id && isset($order_details->sale_no)) {
        ...
        $outlet_row = $this->db->get_where('tbl_outlets', array('id' => $outlet_id))->row();
        $data['company_id'] = ($outlet_row && isset($outlet_row->company_id)) ? $outlet_row->company_id : 1;
        ...
        if($sale_id>0){
            ...
            $single_table_information = $this->db->get_where('tbl_sales', array('id' => $sale_id))->row();
            $order_number = ($single_table_information && isset($single_table_information->sale_no)) ? $single_table_information->sale_no : '';
            ...
            $sales_id = $sale_id;
            $sale_info_existing = $this->db->get_where('tbl_sales', array('id' => $sale_id))->row();
            $sale_no = ($sale_info_existing && $sale_info_existing->sale_no) ? $sale_info_existing->sale_no : ((isset($order_details->sale_no) && $order_details->sale_no) ? $order_details->sale_no : str_pad($sales_id, 6, '0', STR_PAD_LEFT));

            // Sync/update tbl_kitchen_sales for KOT printing capability
            $ks_data = $data;
            $ks_data['sale_no'] = $sale_no;
            unset($ks_data['id']);
            $kitchen_sale = $this->db->get_where('tbl_kitchen_sales', array('sale_no' => $sale_no))->row();
            if ($kitchen_sale) {
                $kitchen_sales_id = $kitchen_sale->id;
                $this->db->where('id', $kitchen_sales_id);
                $this->db->update('tbl_kitchen_sales', $ks_data);
            } else {
                $this->db->insert('tbl_kitchen_sales', $ks_data);
                $kitchen_sales_id = $this->db->insert_id();
            }
        }else{
            $query = $this->db->insert('tbl_sales', $data);
            $sales_id = $this->db->insert_id();
            $sale_no = (isset($order_details->sale_no) && $order_details->sale_no) ? $order_details->sale_no : str_pad($sales_id, 6, '0', STR_PAD_LEFT);
            $sale_no_update_array = array('sale_no' => $sale_no);
            $this->db->where('id', $sales_id);
            $this->db->update('tbl_sales', $sale_no_update_array);

            // Insert parent record into tbl_kitchen_sales so getKitchenSaleDetailsBySaleNo finds it for KOT print service
            $ks_data = $data;
            $ks_data['sale_no'] = $sale_no;
            unset($ks_data['id']);
            $this->db->insert('tbl_kitchen_sales', $ks_data);
            ...
            $notification_message = 'New Order: '.$sale_no.' placed via Waiter App';
            $bar_kitchen_notification_data = array(
        ...
        if($sales_id>0 && count($order_details->items)>0){
            foreach($order_details->items as $item){
                ...
                // Also populate tbl_kitchen_sales_details for KOT print capability & Kitchen display
                $kitchen_item_data = $item_data;
                $kitchen_item_data['sales_id'] = $kitchen_sales_id;
                $kitchen_item_data['cooking_status'] = 'New';
                $kitchen_item_data['is_print'] = 1;
                $this->db->insert('tbl_kitchen_sales_details', $kitchen_item_data);
                $kitchen_sales_details_id = $this->db->insert_id();
                if($item->item_previous_id==""){
                    $this->db->where('id', $kitchen_sales_details_id);
                    $this->db->update('tbl_kitchen_sales_details', array('previous_id' => $kitchen_sales_details_id));
                }
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            if (isset($sale_no) && $sale_no) {
                generateAndEnqueueKotPrint($sale_no, $outlet_id);
            }
        }
