...
function userDesignation($user_id) {
    $CI = & get_instance();
    $user_information = $CI->db->query("SELECT designation FROM tbl_users where `id`='$user_id'")->row();
    return isset($user_information->designation) && $user_information->designation ? $user_information->designation : '';
}
...
if ( ! function_exists('checkAccess')) {
    function checkAccess($controller, $function)
    {
        ...
        if ($cached_user_id !== $user_id || $cached_permissions === null) {
            $user_id_clean = intval($user_id);
            $user = $CI->db->query("SELECT id, role, role_id FROM tbl_users WHERE id='$user_id_clean' AND del_status='Live'")->row();
            ...
            if ($user->role == "Admin") {
                $cached_permissions = "ADMIN";
            } else {
                $role_id_clean = intval($user->role_id);
                $getAccess = $CI->db->query("
                    SELECT a.function_name, a.parent_id 
                    FROM tbl_role_access ra 
                    JOIN tbl_access a ON a.id = ra.access_child_id 
                    WHERE ra.role_id = '$role_id_clean' AND ra.del_status = 'Live'
                ")->result();

                $permissions = array();
                if ($getAccess) {
                    foreach ($getAccess as $value) {
                        $permissions[] = $value->function_name . "-" . $value->parent_id;
                    }
                }
                $cached_permissions = $permissions;
                $CI->session->set_userdata('function_access', $permissions);
            }
            $cached_user_id = $user_id;
        }

        if ($cached_permissions === "ADMIN") {
            return true;
        }

        $controllerFunction = $function . "-" . $controller;
        if (is_array($cached_permissions) && in_array($controllerFunction, $cached_permissions)) {
            return true;
        }
...
function generateAndEnqueueKotPrint($sale_no, $outlet_id = null) {
    $CI = & get_instance();
    ...
    $CI->db->select("tbl_kitchens.id");
    $CI->db->from('tbl_kitchen_sales_details');
    $CI->db->join('tbl_food_menus', 'tbl_food_menus.id = tbl_kitchen_sales_details.food_menu_id', 'left');
    $CI->db->join('tbl_kitchen_categories', 'tbl_kitchen_categories.cat_id = tbl_food_menus.category_id', 'left');
    $CI->db->join('tbl_kitchens', 'tbl_kitchens.id = tbl_kitchen_categories.kitchen_id', 'left');
    $CI->db->where("sales_id", $sale_details->id);
    $CI->db->where("tbl_kitchen_categories.del_status", "Live");
    $CI->db->group_by('tbl_kitchens.id');
    $kitchen_rows = $CI->db->get()->result();
    ...
    if (empty($kitchen_ids)) {
        return false;
    }

    $kitchen_id_str = implode(",", $kitchen_ids);
    $printers = $CI->Common_model->getSelectedPrinters($sale_no, $kitchen_id_str);

    $valid_printers = [];
    if ($printers) {
        $date_format = $CI->session->userdata('date_format') ? $CI->session->userdata('date_format') : 'Y-m-d';
        foreach ($printers as $ky => $value) {
            $sale_items = $CI->Common_model->getAllKitchenItemsAuto($sale_details->id, $value->id);
            if (empty($sale_items)) {
                $sale_items = $CI->Common_model->getAllKitchenItems($sale_details->id, $value->id, 2);
            }
            foreach ($sale_items as $single_item_by_sale_id) {
                $modifier_information = $CI->Sale_model->getModifiersBySaleAndSaleDetailsIdKitchen($sale_details->id, $single_item_by_sale_id->sales_details_id);
            ...
            if ($sale_items) {
                ...
                $printers[$ky]->date = escape_output(date($date_format, strtotime($sale_details->sale_date)));
                $printers[$ky]->time_inv = $sale_details->order_time;
                $printers[$ky]->sales_associate = userName($sale_details->user_id);
                $printers[$ky]->user_designation = isset($sale_details->user_id) && $sale_details->user_id ? userDesignation($sale_details->user_id) : '';
                ...
                $previously_printed = $CI->db->where('sales_id', $sale_details->id)->where('is_print', 2)->get('tbl_kitchen_sales_details')->num_rows() > 0;
                ...
                foreach ($sale_items as $item) {
                    ...
                    if ($qty):
                        ...
                        if ($previously_printed && $item->is_print == 1) {
    ...
    if (count($valid_printers) > 0) {
        $data = array(
            'outlet_id' => $outlet_id ? $outlet_id : ($sale_details->outlet_id ? $sale_details->outlet_id : 1),
            'print_type' => 'KOT',
            'content_data' => json_encode($valid_printers),
            'status' => 'pending',
            'created_at' => date('Y-m-d H:i:s')
        );
        $CI->db->insert('tbl_print_queue', $data);
        return true;
    }
    return false;
}
