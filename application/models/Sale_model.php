...
class Sale_model extends CI_Model {
    ...
    public function getAllFoodMenus(){
      ...
      $result = $this->db->query("SELECT fm.*,fmc.category_name, 0 as item_sold, kt_cat.kitchen_id, k.name as kitchen_name 
      FROM tbl_food_menus fm  
      LEFT JOIN (select id, category_name from tbl_food_menu_categories where del_status='Live') fmc ON fmc.id = fm.category_id 
      ...
      LEFT JOIN (select id, name from tbl_kitchens where del_status='Live') k ON k.id = kt_cat.kitchen_id
    ...
    {
        $this->db->select("tbl_kitchen_sales.*, tbl_kitchen_sales.id as id");
        $this->db->from('tbl_orders_table');
        $this->db->join('tbl_kitchen_sales', 'tbl_kitchen_sales.id = tbl_orders_table.sale_id');
        $this->db->where("tbl_orders_table.table_id", $table_id);
