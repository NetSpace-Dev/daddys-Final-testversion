            ...
            <div id="receipt-data">
               ...
               <p><?php echo lang('date'); ?>:<?= escape_output(date($this->session->userdata('date_format'), strtotime($sale_object->sale_date))); ?>
                   ...
                   <?php $customer = getCustomerData($sale_object->customer_id);?>
                    <?php echo lang('Sales_Associate'); ?>: <?php echo escape_output(userName($sale_object->user_id)) ?><br>
                    ...
                    <br /><?php echo lang('address'); ?>:<?php echo escape_output("$customer->address"); ?>
                    <?php } ?>
                    <?php
                ...
                <table class="table table-condensed">
                    ...
                    <tfoot>
                        ...
                        ?>
                        <?php
                        if(($sale_object->delivery_charge && $sale_object->delivery_charge!="0.00") || (isset($sale_object->delivery_charge_actual_charge) && $sale_object->delivery_charge_actual_charge && $sale_object->delivery_charge_actual_charge!="0.00" && $sale_object->delivery_charge_actual_charge!="0")):
                        ?>
                        <tr>
                           <th>
                               <?php 
                               $charge_label = "Charge";
                               if (isset($sale_object->charge_type) && strtolower($sale_object->charge_type) == 'service') {
                                   $charge_label = "Service Charge";
                               } else if (isset($sale_object->charge_type) && strtolower($sale_object->charge_type) == 'delivery') {
                                   $charge_label = "Delivery Charge";
                               } else {
                                   $charge_label = lang($sale_object->charge_type);
                               }
                               echo escape_output($charge_label);
                               ?>
                           </th>
                            <th class="text-right">
                                <?php 
                                $charge_amount_val = ($sale_object->delivery_charge && $sale_object->delivery_charge!="0.00") ? $sale_object->delivery_charge : (isset($sale_object->delivery_charge_actual_charge) ? $sale_object->delivery_charge_actual_charge : 0);
                                echo escape_output((getPlanTextOrP($charge_amount_val))); 
                                ?>
                            </th>
                        </tr>
                        <?php
