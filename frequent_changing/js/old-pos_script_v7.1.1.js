...
// purpose of this function is to delete data from future sales table by using data from indexeddb
  ...
  function getCurrentDate() {
    ...
    function print_kot_print(order_info, sale_id,kot_print) {
        ...
        let inv_p_table = (order.orders_table_text != undefined && order.orders_table_text)?` <h4  style="margin: 0px">`+order.orders_table_text+ `</h4>`: "";
        ...
        let main_kitchen = (typeof kitchens !== 'undefined') ? kitchens.find(k => k.kitchen_name.toLowerCase().trim() === 'main kitchen') : null;
        let main_kitchen_id = main_kitchen ? Number(main_kitchen.kitchen_id) : null;
        let main_kitchen_name = main_kitchen ? main_kitchen.kitchen_name : 'Main kitchen';
        ...
        // If there are any items in the order, and the Main Kitchen is not yet in the array, initialize it
        let has_any_kitchen_items = false;
        for (let key in order.items) {
            if (Number(order.items[key].kitchen_id)) {
                has_any_kitchen_items = true;
                break;
            }
        }
        if (has_any_kitchen_items && main_kitchen_id && getExistKitchen(main_kitchen_id, total_kitchen_arr)) {
            total_kitchen_arr.push(main_kitchen_id);
            let html_div = '<table data-kitchen_name="' + main_kitchen_name + '" class="kot_container kot_container_' + main_kitchen_id + '"></table>';
            $(".current_kot_items").append(html_div);
        }
        ...
        for (let key in order.items) {
            ...
            if(if_kot_print){
                ...
                if(Number(this_item.kitchen_id)){
                    ...
                    if (kot_print != 2 && main_kitchen_id && Number(this_item.kitchen_id) !== main_kitchen_id) {
                        $(".kot_container_" + main_kitchen_id).append(row_of_item);
                    }
                    sn_counter();
                }
            }
          ...
          if(sale_id == 0){
              ...
              request.onsuccess = function(event) {
                  ...
                  setTimeout(function(){
                      ...
                      let order_el = $(".running_order_order_number").filter(function() {
                          return $(this).text().trim() === sale_no_new;
                      }).closest(".single_order");
                      let active_sale_id = last_inserted_id;
                      if (order_el.length > 0) {
                          order_el.click();
                          let parsed_id = order_el.attr("id").substr(6);
                          if (parsed_id) {
                              active_sale_id = parsed_id;
                          }
                      } else {
                          $("#order_"+last_inserted_id).click();
                      }

                      let order_type = Number(sale.order_type);
                      if(action_type==2 && pre_or_post_payment!=2 && order_type==1){
                          if (print_type_kot == "web_browser") {
                               $(".print_kot_button").click();
                               $(".kot_btn_class").eq(0).click();
                          }
                      }else{
                          if(pre_or_post_payment==2 || is_direct_sale_check==1 || order_type==2 || order_type==3){
                  ...
                  setTimeout(function(){
                      ...
                      let anim_sale_id = last_inserted_id;
                      if (order_el.length > 0) {
                          let parsed_id = order_el.attr("id").substr(6);
                          if (parsed_id) {
                              anim_sale_id = parsed_id;
                          }
                      }
                      createAnimation(anim_sale_id);
                  }, 1100);
  ...
  function get_details_of_a_particular_hold(hold_id, this_action) {
    $.ajax({
      ...
      success: function (response) {
        ...
        $.each(JSON.parse(response.tables_booked), function (key_t, value_t) {
          let table_name = $("#sit_name_" + value_t.table_id).text();
          let table_book_row = "";
          table_book_row +=
            '<div class="single_row fix new_book_to_table" data-name="' + table_name + '" id="new_order_table_' +
            value_t.table_id +
            '">';
          table_book_row +=
      ...
      $.ajax({
          ...
          success: function (response) {
              ...
              for (let key1 in get_waiter_orders_for_update_receiver) {
                  ...
                  let current_sale_no = order.sale_no;
                  updateOrderForWaiter(current_sale_no, order.self_order_content);
                  setOrderInvoiceUpdated(order.id,2);

                  // Auto Print KOT for waiter modified synced orders
                  let print_type_kot = $(".print_type_kot").val();
                  if (print_type_kot !== "web_browser") {
                      $.ajax({
                          url: base_url + "Authentication/get_kot_print_data_by_sale_no_ajax",
                          method: "POST",
                          dataType: "json",
                          data: {
                              sale_no: current_sale_no,
                              csrf_irestoraplus: csrf_value_
                          },
                          success: function (printData) {
                              if (printData && printData.status === 'success' && printData.printer_server_url) {
                                  $.ajax({
                                      url: printData.printer_server_url + "print_server/irestora_printer_server.php",
                                      method: "POST",
                                      dataType: "json",
                                      data: {
                                          content_data: JSON.stringify(printData.content_data),
